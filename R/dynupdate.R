						...
						pimethod = c("parametric", "nonparametric"), B = 1000)
{
    ...
    if (interval == FALSE) {
        ...
        if(method == "block") 
        {
            ...
            dummy = forecast.ftsm(ftsm(fts(1:p, datamatrix), order = order, ngrid = ngrid, method = pcdmethod),
        ...
        else 
        {
            ...
            if(method == "ols") 
            {
                if(length(newdata) == 1)
                {
                    ols = t(ginv(t(base1) %*% base1) %*% t(base1)) %*% newdata
                }
                if(length(newdata) > 1)
                {
                    ols = ginv(t(base1) %*% base1) %*% t(base1) %*% newdata
                }
                forecasts = base2 %*% ols
            }
            if(method == "pls") 
            {
                ...
                if(length(newdata) == 1)
                {
                	pls = ginv(t(matrix(base1,nrow=1)) %*% matrix(base1,nrow=1) + lambda * I) %*% (t(matrix(base1,nrow=1)) %*% newdata + lambda * fore)
                }
                if(length(newdata) > 1)
                {
                	pls = ginv(t(base1) %*% base1 + lambda * I) %*% (t(base1) %*% newdata + lambda * fore)
                }
                forecasts = base2 %*% pls
            }
            if (method == "ridge") {
                ...
                if(length(newdata) == 1)
                {
                	ridg = ginv(t(matrix(base1,nrow=1)) %*% matrix(base1,nrow=1) + lambda * I) %*% (t(matrix(base1,nrow=1)) %*% newdata)
                }
                if(length(newdata) > 1)
                {
	                ridg = ginv(t(base1) %*% base1 + lambda * I) %*% (t(base1) %*% newdata)
          		}
        ...
        if(method == "block") 
        {
            ...
            dummy = forecast.ftsm(ftsm(fts(1:p, datamatrix), order = order, ngrid = ngrid, 
            						method = pcdmethod), h = 1, method = fmethod, 
            						level = level, pimethod = pimethod)
